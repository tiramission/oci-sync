...
| `github.com/spf13/viper` | v1.21.0 | 配置管理（环境变量 + 配置文件）|
...
| 参数 | 必选 | 说明 |
|------|------|------|
| 无 | 否 | 直接使用配置的 repository 列出所有 tags |
...
| `experimental.repo` | `OCI_SYNC_EXPERIMENTAL_REPO` | - | 远程仓库，格式：`<registry>/<repository>`，不包含 tag |

### experimental environment variable

| 变量名 | 必选 | 说明 |
|------|------|------|
| `OCI_SYNC_EXPERIMENTAL_ENABLED` | 否 | 是否启用实验性命令（默认: true）|
| `OCI_SYNC_EXPERIMENTAL_REPO` | `x push/pull/list/delete` 时必选（除非在配置文件中配置）| 远程仓库，格式：`<registry>/<repository>`，不包含 tag |
